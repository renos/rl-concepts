...
class Env(BaseClass):
    ...
    def reset(self, return_info=False):
        ...
        self._last_food = self._player.inventory["food"]
        self._last_drink = self._player.inventory["drink"]
        self._last_energy = self._player.inventory["energy"]
        ...
        if not return_info:
            ...
            dead = False
            unlocked = set()
            return self._obs(), self.return_info(reward, dead, unlocked)
    ...
    def step(self, action):
        ...
        if not self._reward:
            reward = 0.0
        return obs, reward, done, self.return_info(reward, dead, unlocked)

    def survival_reward(self):
        reward = 0
        if self._player.inventory["energy"] < 3:
            reward += (
                max(self._player.inventory["energy"] - self._last_energy, 0) / 10.0
            )
        self._last_energy = self._player.inventory["energy"]
        if self._player.inventory["food"] < 3:
            reward += max(self._player.inventory["food"] - self._last_food, 0) / 10.0
        self._last_food = self._player.inventory["food"]

        if self._player.inventory["drink"] < 3:
            reward += max(self._player.inventory["drink"] - self._last_drink, 0) / 10.0
        self._last_drink = self._player.inventory["drink"]

        # if self._player.inventory["drink"] < 2:
        #     reward -= 0.05
        # if self._player.inventory["food"] < 2:
        #     reward -= 0.05
        # if self._player.inventory["energy"] < 2:
        #     reward -= 0.05
        return reward

    def return_info(self, reward, dead, unlocked):
        info = {
    ...
    def _obs(self):
        return (
            self.render()
            if self.use_pixels
            else np.zeros_like(self.observation_space.shape)
        )
...
class EnvSample(BaseClass):
    ...
    def reset(self, return_info=False):
        ...
        self._player = objects.Player(self._world, center)
        self._last_health = self._player.health
        self._last_food = self._player.food
        self._last_drink = self._player.drink
        self._world.add(self._player)
        self._unlocked = set()
        worldgen.generate_world(self._world, self._player)
        if not return_info:
